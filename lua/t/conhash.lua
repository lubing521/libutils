...
----------------------------conhash test------------------------------------
-----------------------------------------------------------------------------
conhash = utils_lua.conhash_init()
...
for  i, v in pairs(ips) do
	print((i - 1) .. " " .. v)
	rt = utils_lua.conhash_add_node(conhash, i - 1, v, 500)
	if rt==-1 then
		print("add " .. v .. "failed")
	end
end
...
rt = utils_lua.conhash_del_node(conhash, "10.10.10.10", string.len("10.10.10.10"))
if rt==-1 then
...
print("look up")
ip = "200.200.200.200"
ip = utils_lua.conhash_lookup(conhash, ip, string.len(ip))
